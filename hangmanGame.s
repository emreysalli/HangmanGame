# Emre Yasin Şallı 190701051
# Tamay Karagülle 190701113
# Qtspim simülatörüyle çalıştırabilirsiniz
# Oyunu başlattığınızda sizden bir sayı(1-6) girmenizi ister sayıya göre kelime karşınıza gelir ardından her seferinde tek harf girişi yaparak kelimeyi tahmin edebilirsiniz
.data
...
.text
    ...
    isContains:
        addi $v0, $zero, 1 # kelimede aranan karakter var
    containsWhileEnd:
        lw $a0, 0($sp)
        addi $sp, $sp, 4
        jr $ra
    ...
    again:
        # kullanıcının oyunu tekrar oynamak isteğine göre oyunu tekrar başlatan ya da bitiren fonksiyon
        la $a0, playAgain
        jal print
    ...
    print:
        # konsola string yazdıran fonksiyon
        addi $sp, $sp, -4
        sw $ra, 0($sp)
    ...
    printInt:
        # konsola integer yazdıran fonksiyon
        addi $sp, $sp, -4
        sw $ra, 0($sp)
